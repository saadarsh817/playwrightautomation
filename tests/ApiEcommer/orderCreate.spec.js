...
test('should fail to create order with invalid productOrderedId', async () => {
    const createOrder = await apiContext.post("https://rahulshettyacademy.com/api/ecom/order/create-order", {
        headers: {
            'Authorization': token,
            'Content-type': 'application/json'
        },
        data: { orders: [{ country: "Cuba", productOrderedId: "invalid_id" }] }
    });
...
test('should fail to create order with missing country', async () => {
    const createOrder = await apiContext.post("https://rahulshettyacademy.com/api/ecom/order/create-order", {
        headers: {
            'Authorization': token,
            'Content-type': 'application/json'
        },
        data: { orders: [{ productOrderedId: "67a8df1ac0d3e6622a297ccb" }] }
    });
    expect(createOrder.status()).toBe(400);
    const responseBody = await createOrder.json();
    expect(responseBody).toHaveProperty('message');
...
test('should fail to create order with empty orders array', async () => {
    const createOrder = await apiContext.post("https://rahulshettyacademy.com/api/ecom/order/create-order", {
        headers: {
            'Authorization': token,
            'Content-type': 'application/json'
...
test('should fail to create order without authorization', async () => {
    const createOrder = await apiContext.post("https://rahulshettyacademy.com/api/ecom/order/create-order", {
        headers: {
            'Content-type': 'application/json'
        },
